...
//		}
	 ...
	 * 配置所有节点CephConfEnv
	 */
	public static void compileNodeConfEnvDemo() {
		System.out.println("==============compileNodeConfEnv Demo==============");
		...
		Map<String, AsyncEntity> entityMap = new HashMap<>(hosts.size());
		try {
			for (Map.Entry<String, CommandExecuteResult> entry : GlobalCacheSDK.compileNodeConfEnv(hosts).entrySet()) {
				if (entry.getValue().getStatusCode() == StatusCode.SUCCESS) {
					entityMap.put(entry.getKey(), (AsyncEntity) entry.getValue().getData());
				} else {
					System.out.println("接口调用失败");
